...
$('clearAudio').addEventListener('click', () => { audioPlayer.removeAttribute('src'); audioPlayer.load(); audioFileInput.value = ''; recordingDuration = 0; transcriptSegments = []; history = []; future = []; $('fileName').textContent = 'No recording loaded'; $('documentName').textContent = 'Untitled recording'; $('sessionName').textContent = 'Not started'; $('sessionStatus').textContent = 'Upload audio to begin'; render(); setStatus('Recording and transcript cleared.'); }); $('generateSample').addEventListener('click', loadSample); $('parseTranscript').addEventListener('click', parseTranscript); $('undoButton').addEventListener('click', undo); $('redoButton').addEventListener('click', redo);
