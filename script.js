...
const parseTranscript = () => { const lines = $('rawTranscript').value.trim().split(/\r?\n/).map((line) => line.trim()).filter(Boolean); const parsed = []; for (let index = 0; index < lines.length; index += 3) { const range = lines[index + 1]?.match(/(.+?)\s*-->\s*(.+)/); if (!range) continue; const startMs = parseTimestamp(range[1]); const endMs = parseTimestamp(range[2]); if (startMs !== null && endMs !== null) parsed.push(createSegment(startMs, endMs, lines[index], lines[index + 2] || '')); } if (!parsed.length) { setStatus('Use speaker, timestamp range, then text on separate lines.', true); return; } recordingDuration = Math.max(recordingDuration, parsed.at(-1).endMs); loadSegments(parsed, 'Transcript imported.'); };
const addSegment = () => { const startMs = transcriptSegments.at(-1)?.endMs || 0; pushHistory(); transcriptSegments.push(createSegment(startMs, startMs + 5000, 'Male_1', 'New segment')); render(); setStatus('Segment added.'); };
...
audioPlayer.addEventListener('timeupdate', () => { $('currentTime').textContent = formatTimestamp((audioPlayer.currentTime || 0) * 1000, true); document.querySelectorAll('.segment').forEach((card, index) => card.classList.toggle('playing', Number(card.dataset.start) <= audioPlayer.currentTime * 1000 && Number(transcriptSegments[index + 1]?.startMs || Infinity) > audioPlayer.currentTime * 1000)); });
...
$('transcribeButton').addEventListener('click', async () => { const file = audioFileInput.files[0]; if (!file) { setStatus('Upload a recording before transcribing.', true); return; } $('transcribeButton').disabled = true; $('sessionStatus').textContent = 'Gemini transcription and speaker detection in progress'; setStatus('Gemini is transcribing and detecting speakers...'); try { const form = new FormData(); form.append('audio', file); const response = await fetch('/api/transcribe', { method: 'POST', body: form }); const result = await response.json(); if (!response.ok) throw new Error(result.error); if (result.segments?.length) recordingDuration = Math.max(recordingDuration, result.segments.at(-1).endMs || 0); loadSegments(result.segments || [], 'Gemini transcript ready.'); $('sessionStatus').textContent = 'Gemini transcript ready · speakers detected'; } catch (error) { $('sessionStatus').textContent = 'Gemini transcription failed'; setStatus(error.message || 'Transcription failed.', true); } finally { $('transcribeButton').disabled = false; } });
